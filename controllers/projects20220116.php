...
class ProjectsController extends Controller {
  ...
  function add($args = array()) {
    ...
    if ($step == 1) {
      ...
      if ($step == 4 & is_arr_valid($critical_activity)) {
  ...
  function bulk_api_status($args = array()) {
    ...
    if (is_arr_valid($_POST)) {
      ...
      $this->{$this->_model_name}->set_public_projects($public_projects);
    }  
    ...
    $this->set_var("public_projects", $this->{$this->_model_name}->get_public_projects());
    $this->render();
  } // bulk_api_status()
